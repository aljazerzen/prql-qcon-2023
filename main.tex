...
\institute{\large Compiler developer @EdgeDB}
...
Tuples: \begin{verbatim}{ {my_int = 5, 4.2, my_bool = true} }\end{verbatim}

- named fields,
- different types,
- static number of fields

Arrays: \begin{verbatim}{ [1, 2, 10, -3] }\end{verbatim}
...
% Because our `invoices` relation is actaully stored in database table,
...
% And this is an ideomatic PRQL query.
% Built on a few basic syntactic and semantic rules.
% With formal specifiation that would probably fit on a few pages.
...
- \textbf{\texttt{filter}} will not change columns

- \textbf{\texttt{derive}} \& \textbf{\texttt{select}} will not change number of rows

% this is not true for SQL clauses: SELECT may also imply aggregation

- \textbf{\texttt{aggregate}} will produce exactly one row
...
% sum is an agggregation function
...
% well it could, but still not all SQL statments (REINDEX, CREATE, ...)
...
\includegraphics[width=\textwidth]{images/error_message.png}
...
% even worse: affiliation with VC would have negative impact on adopton
